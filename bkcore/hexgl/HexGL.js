...
bkcore.hexgl.HexGL = function (opts) {
  ...
  /******************** Modified by LGE ***********************/
  // Game Mode selector (Hand Hold or Streeing Wheel mode)
  this.gameMode = opts.gameMode == undefined ? 0 : opts.gameMode;
  // For assigining mrcuService instance
  this.mrcuService = null;
  // Create BleGattService instance
  this.blegattService = new bkcore.hexgl.BleGattService();
  // scan ble devices and connect bHaptics' gaming vest
  this.blegattService.isBleEnabled();
  /************************************************************/
...
bkcore.hexgl.HexGL.prototype.load = function (opts) {
  this.track.load(opts, this.quality, this.difficulty);
};
...
bkcore.hexgl.HexGL.prototype.displayScore = function (f, l) {
  ...
  if (this.gameover !== null) {
    ...
    /******************** Modified by LGE ***********************/
    // Switch Cursor display to visible,
    window.webOSSystem.setCursor("default");

    // Cancel getSensorEventData API subscribe when display score screen
    this.mrcuService.cancelSensorDataSubscribe();

    // Remove vibration when gameover
...
bkcore.hexgl.HexGL.prototype.tweakShipControls = function () {
  ...
  if (this.difficulty == 1) {
    c.airResist = 0.035;
    c.airDrift = 0.07;
    c.thrust = 0.03; // Original 0.035
    c.airBrake = 0.025; // Original 0.04
    c.maxSpeed = 7.0; // Original 9.6
    c.boosterSpeed = c.maxSpeed * 0.35;
    c.boosterDecay = 0.007;
    c.angularSpeed = 0.015; // Original 0.0140
    c.airAngularSpeed = 0.015; // Original 0.0165
    c.rollAngle = 0.7; // Original 0.6
    c.shieldDamage = 0.03;
    ...
    c.airResist = 0.03; // Original 0.02
    c.airDrift = 0.06;
    c.thrust = 0.015; // Original 0.02
    c.airBrake = 0.025;
    c.maxSpeed = 5.0; // Original 7.0
