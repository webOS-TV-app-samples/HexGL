...
bkcore.hexgl.ShipControls = function (ctx) {
  ...
  /******************** Modified by LGE ***********************/
  // Create BleGattService instance
  this.blegattService = new bkcore.hexgl.BleGattService();
  /************************************************************/
  ...
  /******************** Modified by LGE ***********************/
  // Disable below code because leap controller & gamePad is unused
  ...
  // Game Mode & Controller Selector
  this.gameMode = ctx.gameMode;
  this.gameController = ctx.controlType == undefined ? 0 : ctx.controlType;

  // Keyboard controller
  // Add keyBoardControl function to distinguish from mrcuControl
  const keyBoardControlStart = () => {
  ...
  // Color Key Event Controller
  window.addEventListener("keydown", (e) => {
    // Keycodes in MRCU input events mean: Yellow(405) Blue(406)
    ...
    switch (e.keyCode) {
      ...
      case 405:
        ctx.mrcuService.toggleCursorDisplay();
        break;

      // Press blue button then show cursor & cancel SensorDataSubscribe for restart game
      case 406:
        window.webOSSystem.setCursor("default");
        ctx.mrcuService.cancelSensorDataSubscribe();
        window.location.reload();
        break;
    }
  });

  // getSensorEventData for Motion Sensor Controller
  if (this.gameController == 0) {
    // Switch the cursor display to invisible when the game starts
    window.webOSSystem.setCursor("./assets/cursor_blank_image.png");
    ctx.mrcuService.init({
      gameMode: this.gameMode,
      key: this.key
    });
    // getSensorEventData with game start
    ctx.mrcuService.getSensorEventData();
  }

  // Using Keyboard Controller
  else if (this.gameController == 1) {
    // Switch the cursor display to invisible when the game starts
    window.webOSSystem.setCursor("./assets/cursor_blank_image.png");
    keyBoardControlStart();
  }
  /************************************************************/
};
...
bkcore.hexgl.ShipControls.prototype.destroy = function () {
  ...
  /********************Modified by LGE***********************/
  this.blegattService.createVibration(Vibration.all);
  /**********************************************************/
...
bkcore.hexgl.ShipControls.prototype.fall = function () {
  /********************Modified by LGE***********************/
  // Cancel subscribe when gameover
  this.blegattService.createVibration(Vibration.fall);
  /**********************************************************/
...
bkcore.hexgl.ShipControls.prototype.update = function (dt) {
  ...
  // Disable below code because leap controller is unused
  /*
  ...
  if (this.active) {
    // const value is angle that is optimized with game mode
    const HANDHOLD_MAX_ANGLE = 60;
    const WHEEL_MAX_ANGLE = 45;

    // Function that calculate the angularAmount and rollAmount according to the MRCU angle
    const handleAngularValue = (maxAngle) => {
      if (this.key.left) {
        if (this.key.angle < maxAngle) {
          angularAmount +=
            ((this.angularSpeed * this.key.angle) / maxAngle) * dt;
          rollAmount -= (this.rollAngle * this.key.angle) / maxAngle;
        } else {
    ...
    // gameController -- 0 : Motion Sensor / 1 : KeyBoard
    // gameMode -- 0 : handhold / 1 : steering wheel
    if (this.gameController == 0) {
      if (this.gameMode == 0) {
        handleAngularValue(HANDHOLD_MAX_ANGLE);
      } else if (this.gameMode == 1) {
        handleAngularValue(WHEEL_MAX_ANGLE);
      }
    } else if (this.gameController == 1) {
      if (this.key.left) {
        angularAmount += this.angularSpeed * dt;
    ...
    // Disable below code because leap / touch / gamepad controller is unused
  ...
  this.movement.z += this.speed * 1 * dt;
...
bkcore.hexgl.ShipControls.prototype.boosterCheck = function (dt) {
  ...
  if (color.r == 255 && color.g < 127 && color.b < 127) {
    ...
    /********************Modified by LGE***********************/
    this.blegattService.createVibration(Vibration.boost);
    /**********************************************************/
  }
...
bkcore.hexgl.ShipControls.prototype.collisionCheck = function (dt) {
  ...
  if (collision.r < 255) {
    bkcore.Audio.play("crash"); // Play Crash Sound
    ...
    if (rCol > lCol) {
      ...
      /********************Modified by LGE***********************/
      this.blegattService.createVibration(Vibration.left);
      /**********************************************************/
    } else if (rCol < lCol) {
      // Repulse left
      this.repulsionForce.x += this.repulsionAmount;
      ...
      /********************Modified by LGE***********************/
      this.blegattService.createVibration(Vibration.right);
      /**********************************************************/
    } else {
      ...
      this.speed = 0;
      /********************Modified by LGE***********************/
      this.blegattService.createVibration(Vibration.front);
      /**********************************************************/
    }
    ...
    /********************Modified by LGE***********************/
    this.blegattService.createVibration(Vibration.clear);
    /**********************************************************/
    return false;
  }
};
